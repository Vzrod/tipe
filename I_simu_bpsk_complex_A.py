# -*- coding: utf-8 -*-
"""
Simulation BPSK — refactor en enveloppe complexe (baseband equivalent).

Convention de chaîne :
    bits ─► map ─► sym ─► upsample ─► s_bb ──────┐
                  (complexe)     (complexe)       │
                                                  ▼   to_passband = Re{ · e^{j2πfc t} }
                                                  s  (RÉEL, sur le canal)
                                                  │
                                                  ▼   + bruit AWGN réel
                                                  r  (RÉEL)
                                                  │
                                                  ▼   to_baseband = 2·r·e^{-j2πfc t}
                                                  r̃  (COMPLEXE, contient harm. 2fc)
                                                  │
                                                  ▼   filtre adapté + échantillonnage
                                              r_sym  (COMPLEXE, 1 par symbole)
                                                  │
                                                  ▼   décision : signe de Re(·) pour BPSK
                                                  bits_r

Relation fondamentale (modulation linéaire) :
...
# =============================================================
# 1) Briques génériques (réutilisables BPSK / QPSK / M-QAM)
# =============================================================

def upsample(sym, L):
    """Étale chaque symbole sur L échantillons (mise en forme NRZ rectangulaire)."""
    return np.repeat(sym, L)


def to_passband(s_bb, fc, fs):
    """Enveloppe complexe → signal RÉEL passband.
        s(t) = Re{ s_bb(t) · exp(j 2π fc t) }
    """
...
def matched_filter_sample(r_bb, L):
...
def simu_bpsk(bits, SNRbdB, Lc, Nc, fc, plots=True):
    ...
    s_bb = upsample(sym, L)           # complexe, fs échantillons/s
    ...
    r_sym    = matched_filter_sample(r_bb_raw, L) # complexe, 1 par bit
